...
int xhci_halt(struct xhci_hcd *xhci)
{
	...
	if (ret) {
		if (!(xhci->xhc_state & XHCI_STATE_DYING))
			...
			/* Try to recover the controller by resetting it a few times.
			 * This helps on some platforms where the HC doesn't respond
			 * to halt but a soft reset can recover internal state.
			 */
			xhci_info(xhci, "Attempting HC reset recovery\n");
			for (i = 0; i < retry; i++) {
