...
class ELF:
	...
	def find_section_names(self):
		...
		if len(__str_offset_sechdr_lst) != 1 or len(__str_offset_sechdr_lst[0][1]) != 1:
			log_e(f"[WARN] ELF: could not find .shstrtab normally in {self.elf_file_name}, trying fallback...")

			for __shdr in self.shdr:
				if __shdr.sh_type == SHT_STRTAB:
					__start_bin_idx = __shdr.sh_offset
					__end_bin_idx = __shdr.sh_offset + __shdr.sh_size
					sec_data = self.binary_img[__start_bin_idx:__end_bin_idx]
					if b".strtab" in sec_data or b".symtab" in sec_data:
						log_e(f"[INFO] ELF: using fallback string table for section names in {self.elf_file_name}")
						__str_offset_sechdr_lst = [[__shdr, [0]]]
						break

			if len(__str_offset_sechdr_lst) == 0:
				log_e(f"[ERROR] ELF: section name string search failed for {self.elf_file_name}")
				raise RuntimeError

		__sh_strtab = __str_offset_sechdr_lst[0][0]
		for __shdr in self.shdr:
			__shdr.sh_set_name(
				self.binary_img[__sh_strtab.sh_offset:__sh_strtab.sh_offset + __sh_strtab.sh_size]
			)
